...
const Dashboard = () => {
    ...
    const handleMarkPaid = async (sub) => {
        ...
        if (sub.billingCycle === 'Monthly') newDate.setMonth(newDate.getMonth() + 1);
            ...
            const days = parseInt(sub.billingCycle);
            if (!isNaN(days)) newDate.setDate(newDate.getDate() + days);
        }
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-white dark:bg-gray-900 rounded-3xl p-8 w-full max-w-md shadow-2xl border border-gray-200 dark:border-gray-700 animate-slide-up">
                        ...
                        <form onSubmit={handleUpdate} className="space-y-4">
                            ...
                            <button type="submit" className="w-full py-3.5 mt-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg">Save Changes</button>
                        </form>
                    </div>
                </div>
            )}
